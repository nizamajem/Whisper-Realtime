...
import numpy as np
import sys
# define global variable to store transcript data
transcript_data = []
...
def transcribe_forever(audio_queue, result_queue, audio_model, english, verbose, save_file):
    while True:
        ...
        if not verbose:
            ...
            transcript_data.append(predicted_text)
            print(transcript_data)
            user_input = input("Tekan 'q' untuk keluar atau 'Enter' untuk melanjutkan: ")
            if user_input.lower() == 'q':
                full_traksript = ' '.join(transcript_data)
                with open('hasil.txt', 'w') as file:
                    file.write(full_traksript)
                sys.exit()
        else:
